...
<img src="results/figures/evaluation_metrics.png" width="500"/> 
...
To understand what visual regions the model focuses on, an **explainability heatmap** is generated by overlaying the mean pixel intensity of an input image as a jet-coloured heatmap on top of the original image.

| Aspect | Detail |
|---|---|
| Method | Mean channel intensity overlay (jet colormap, alpha=0.5) |
| Input | First frame of a sample story sequence |
| Purpose | Highlights which spatial regions carry the most visual signal |
| Output | `results/figures/explainability_heatmap.png` |

**How it works:**

1. A sample input image is taken from the first frame of a story sequence
2. The image tensor `(H, W, C)` is averaged across the colour channels to produce a 2D intensity map
3. This intensity map is overlaid on the original image using a `jet` colourmap at 50% transparency
4. Brighter/warmer regions (red/yellow) indicate higher average pixel intensity — areas the model's encoder is most sensitive to
5. Cooler regions (blue) indicate lower activation areas

<img src="results/explainability_heatmap.png" width="500"/> 
<img src="results/sequence_feature_heatmap.png" width="500"/> 
...
<img src="results/figures/ablation_study.png" width="500"/> 
...
<img src="results/sample_sequence_visualization.png" width="500"/> 
<img src="results/prediction_example.png" width="500"/> 
...
|---|---|---|
| 1 | Drive mount + folder setup | Creates project directory structure |
| 2 | Dataset loading | Downloads StoryReasoning from HuggingFace |
| 3 | Data exploration | Visualises sample stories and image-text pairs |
| 4 | DataLoader setup | Wraps dataset into PyTorch `Dataset` and `DataLoader` |
| 5 | Model definition | Builds `BaselineModel` (ResNet-18 + LSTM) |
| 6 | Training | Trains for 10 epochs, logs loss per epoch |
| 7 | Evaluation | Computes Feature MSE and Cosine Similarity |
| 8 | Visualisation | Generates PCA plot, heatmaps, ablation chart |
| 9 | Save outputs | Saves model weights, CSVs, figures to Drive |
